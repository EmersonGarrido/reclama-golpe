...
export default function DashboardPage() {
  ...
  const fetchUserData = async (userId: string) => {
    try {
      ...
      const statsResponse = await fetch(`http://localhost:3333/users/${userId}/stats`, {
        headers: {
      ...
      const scamsResponse = await fetch(`http://localhost:3333/scams?userId=${userId}&limit=5`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Link href="/painel/denuncias" className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4">
              <div className="bg-red-100 p-3 rounded-lg">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">Minhas Denúncias</h3>
                <p className="text-sm text-gray-600 mt-1">Gerencie todas as suas denúncias</p>
              </div>
            </div>
          </Link>

          <Link href="/painel/comentarios" className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4">
              <div className="bg-blue-100 p-3 rounded-lg">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">Meus Comentários</h3>
                <p className="text-sm text-gray-600 mt-1">Veja seus comentários recentes</p>
              </div>
            </div>
          </Link>

          <Link href="/painel/salvos" className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">Denúncias Salvas</h3>
                <p className="text-sm text-gray-600 mt-1">Denúncias que você salvou</p>
              </div>
            </div>
          </Link>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900">Denúncias Recentes</h2>
              <Link href="/painel/denuncias" className="text-red-600 hover:text-red-700 text-sm font-medium">
                Ver todas →
              </Link>
            </div>
          </div>
                ...
                <div key={scam.id} className="p-6 hover:bg-gray-50">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <Link href={`/golpe/${scam.id}`} className="text-lg font-semibold text-gray-900 hover:text-red-600">
                        {scam.title}
                      </Link>
